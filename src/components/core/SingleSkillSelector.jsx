  ...
  componentDidUpdate(prevProps, prevState) {
    if (
      (!_.isEqual(this.state.selected, prevState.selected) ||
        !_.isEqual(this.state.score, prevState.score)) &&
      this.props.onChange
    ) {
    ...
    if (!_.isEqual(this.props.skill, prevProps.skill)) {
      this.setState({
        ...
        ...(this.props.type === "single"
          ? { search: this.props.skill ? this.props.skill.name : "" }
          : {}),
      });
    }
              ...
              <div className="list-group suggestions">
                {(this.props.Skill.skills[this.searchKey()] || []).map(
                  (skill) => {
                    ...
                    if (
                      this.props.excludeSkills.length > 0 &&
                      this.props.excludeSkills.includes(skill.id)
                    ) {
                      return null;
                    }
                    ...
                    return (
                      <a
                        ...
                        key={skill.id}
                        onClick={this.onSelectSkill.bind(this, skill)}
                      >
                        {skill.name}
                      </a>
                    );
